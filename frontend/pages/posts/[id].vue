<template>
  <div class="min-h-screen bg-white dark:bg-gray-950 py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
      ...
      <!-- 文章內容 -->
      <ArticleViewer v-else-if="article" :article="article" :tags="tags" />
    </div>
  </div>
</template>
...
const generateDescription = (content: string): string => {
  // Strip markdown and truncate to ~160 chars
...
// Watch for article changes and update head
watch(article, (newArticle) => {
  if (newArticle) {
    const description = generateDescription(newArticle.content)
    const title = `${newArticle.title} | 部落格`
    ...
    useHead({
      ...
      link: [
        { rel: 'canonical', href: `${useRequestURL().origin}/posts/${newArticle.id}` }
      ]
    })
  }
}, { immediate: true })
